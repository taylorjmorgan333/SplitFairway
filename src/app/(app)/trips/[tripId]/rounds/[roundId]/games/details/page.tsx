...
import { CustomGamesSection, type CustomGameView } from "@/components/rounds/custom-game-section";
...
export default async function GameDetailsPage({
  ...
  const customGames: CustomGameView[] = [];
  ...
  for (const game of gameRows ?? []) {
    ...
    } else if (game.game_type === "custom") {
      customGames.push({
        id: game.id,
        name: game.name,
        dollarValue: game.dollar_value,
        playerNames: participants.map((p) => displayNameById.get(p.round_player_id) ?? "Golfer"),
      });
    }
  }
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <ButtonLink href={`/trips/${tripId}/rounds/${roundId}/games`} variant="ghost" size="sm">
          Games
        </ButtonLink>
      </div>
      ...
      {!hasAnyGames && (
        <p className="text-base text-charcoal-400">
          ...
          {isCaptain ? " — head back to Games to add one." : " for this round."}
        </p>
      )}
      ...
      <div className="space-y-6">
        ...
        />
        <CustomGamesSection tripId={tripId} roundId={roundId} isCaptain={isCaptain} games={customGames} />
      </div>
    </div>
  );
}
