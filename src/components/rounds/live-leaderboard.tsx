...
export function LiveLeaderboard({
  ...
  const [metric, setMetric] = useState<StandingsMetric>("net");
  ...
  const standings = computeStandings(playerScoreInputs, metric);
  ...
  return (
    <div className="space-y-4">
      ...
      {!liveScoreVisibility && !isCaptain && (
        <p className="rounded-lg bg-cream-100 px-3.5 py-2.5 text-xs text-charcoal-500">
      ...
      <div className="overflow-hidden rounded-2xl border border-forest-900/[0.06] bg-white shadow-card">
        {standings.length === 0 ? (
          <p className="px-4 py-6 text-center text-sm text-charcoal-400">
              ...
              return (
                <li key={s.roundPlayerId} className="flex items-center justify-between px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-charcoal-800">{player?.displayName ?? "Golfer"}</p>
                  ...
                  <span className="font-serif text-lg text-forest-900">
                    {s.value}
                    {metric === "stableford" && <span className="ml-1 text-xs font-sans text-charcoal-400">pts</span>}
                  </span>
                </li>
              );
            })}
      ...
      <p className="text-xs text-charcoal-400">
        ...
        shows their gross score instead. Match play and skins come from Games once they&apos;re set up.
      </p>
    </div>
  );
}
